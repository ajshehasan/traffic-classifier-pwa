  ...
  if (done) {
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center space-y-6">
        <div className="text-5xl font-bold text-slate-900">{correct}/{TOTAL}</div>
        <div className="text-slate-500">Quiz complete! Streak: {prefs.streakDays} days</div>
      ...
      {question && (
        <>
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
            <p className="text-sm font-medium text-slate-700">What attack type does this connection represent?</p>
          ...
          <div className={`grid grid-cols-2 gap-3`}>
            {choices.map(cls => {
              ...
              let btn = 'border border-slate-200 text-slate-700 hover:border-slate-400 hover:bg-slate-50'
              if (answered) {
                if (isCorrect) btn = 'border-2 border-green-500 bg-green-50 text-green-700'
                else if (isChosen) btn = 'border-2 border-red-500 bg-red-50 text-red-700'
                else btn = 'border border-slate-200 text-slate-400 opacity-60'
              }
              return (
                <button
                  key={cls}
              ...
              <div className="flex gap-4 flex-wrap text-sm">
                ...
                {modelPred && (
                  <div>
                    ...
                    <span className="text-slate-400 text-xs ml-1">({(modelPred.confidence * 100).toFixed(0)}%)</span>
                  </div>
                )}
              </div>
